...
const isTranscribing = ref(false); // Estado para controlar la transcripcion
...
async function transcribeAudio() {
  ...
  try {
    ...
    // Use 'audio_file' to match the proxy route expectation
    ...
    // Populate the notes above
    notes.value += (notes.value ? "\n\n" : "") + transcribedText.trim();

    showNotification(
      "success",
      "Transcripción lista",
      "El audio ha sido convertido a texto.",
    );
  } catch (error) {
    console.error(error);
    showNotification("error", "Error", "No se pudo transcribir el audio.");
  } finally {
    ...
    >
      ...
      <div>
        <section v-if="!isEmergencyMode" class="border rounded-xl p-4">
          ...
          <div
            class="mb-4 border border-purple-200 bg-purple-50 rounded-xl p-4"
          >
            <p
              class="text-sm font-medium text-purple-800 mb-3 flex items-center gap-1.5"
            >
              <Mic class="w-4 h-4" /> Nota de voz
            </p>

            <!-- Error de permisos -->
            <p v-if="audioError" class="text-xs text-red-500 mb-2">
              {{ audioError }}
            </p>
            ...
            >
              ...
              <span class="text-xs text-gray-400"
                >Mantené el botón para grabar</span
              >
            </div>
            ...
            <div v-if="isRecording" class="flex items-center gap-3">
              ...
              <span
                class="text-sm font-mono text-red-600 font-semibold min-w-[48px]"
              >
            ...
            <div v-if="!isRecording && audioUrl" class="space-y-3">
              ...
              <div class="flex flex-wrap gap-2">
                <!-- Nuevo botón de Transcribir -->
                <button
                  type="button"
                  @click="transcribeAudio"
                  :disabled="isTranscribing"
                  class="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium disabled:opacity-50"
                >
                  {{
